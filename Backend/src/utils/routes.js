...
router.post("/rooms", authenticate, async (req, res) => {
    try {
        ...
        if (existingRoom) {
            return res.status(400).json({ success: false, error: "Uma sala com esse nome já existe." });
        }
...
router.post("/rooms/:roomId/kick", authenticate, async (req, res) => {
    try {
        ...
        if (room.creator.toString() !== req.user.id) {
            return res.status(403).json({ error: "Você não tem permissão para expulsar usuários desta sala" });
        }
